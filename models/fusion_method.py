...
class SumFusion(nn.Module):
    def __init__(self, input_dim=512, output_dim=100,init_weight_x=0.5, init_weight_y=0.5):
        super(SumFusion, self).__init__()
        self.fc_x = nn.Linear(input_dim, output_dim)
        self.fc_y = nn.Linear(input_dim, output_dim)
        self.weight_x = nn.Parameter(torch.tensor(init_weight_x))
        self.weight_y = nn.Parameter(torch.tensor(init_weight_y))
...
class ConcatFusion(nn.Module):
    def __init__(self, input_dim=512, output_dim=100,init_weight_x=0.5, init_weight_y=0.5,sam=False):
        super(ConcatFusion, self).__init__()
        self.fc_out = nn.Linear(input_dim, output_dim)
    def forward(self, x, y):
        return x,y,self.fc_out(torch.cat((x, y), dim=1))
